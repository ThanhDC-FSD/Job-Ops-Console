...
const i18n = {
  en: {
    ...
    crawlAll: 'Crawl All',
    crawlCompanyJobs: 'Crawl Company Jobs',
    crawlCompanyName: 'Company Name',
    crawlCompanyPlaceholder: 'e.g. Google or https://www.linkedin.com/company/google/',
    crawlCompanyRequired: 'Enter a company name before running company jobs crawl.',
    startCrawlFiltered: 'Start Crawl Filtered',
    cancelCrawlFiltered: 'Cancel Crawl',
    ...
    predictCompanyName: 'Company name',
    predictCompanyWebsite: 'Company website',
    predictRecentDays: 'Recent days',
    predictMaxJobs: 'Max jobs',
    predictDbWindowHint: 'Recent days and Max jobs are only used for loading jobs from the database window.',
    predictCustomOnlyHint: 'Custom JD mode is active. Recent days and Max jobs are ignored unless you also provide Job IDs.',
    ...
    artifactRepairPipeline: 'Generated Artifact Repair',
    garbageCleanupPipeline: 'Garbage Cleanup',
    retainDays: 'Retain days',
    maxDeletes: 'Max deletes',
...
function normalizePredictionArray(value) {
...
function appendPredictionTextLine(lines, label, value) {
  const text = String(value || '').trim()
  if (!text) return
  lines.push(`${label}: ${text}`)
}

function appendPredictionListLines(lines, label, items) {
...
function appendPredictionMockPlanLines(lines, mockPlan) {
  if (!Array.isArray(mockPlan) || mockPlan.length === 0) return
  lines.push('Mock plan:')
  mockPlan.forEach((item, idx) => {
    const step = String(item?.step || idx + 1).trim()
    const title = String(item?.title || '-').trim() || '-'
    const durationMinutes = Number(item?.duration_minutes || 0)
    const task = String(item?.task || '').trim()
    lines.push(`- Step ${step}: ${title} (${Number.isFinite(durationMinutes) ? durationMinutes : 0} min)${task ? ` - ${task}` : ''}`)
  })
}
...
function buildPredictionExportText(detail) {
  ...
  const headerWebsite = String(row.company_website || '').trim() || '-'
  const questionCount = Number(row.question_count || qaPack.total_questions || questionCards.length || 0)
  const sourceCount = Number(row.source_document_count || row.source_summary?.source_document_count || (Array.isArray(row.source_summary?.source_documents) ? row.source_summary.source_documents.length : 0) || 0)
  const lines = [
    headerTitle,
    headerCompany,
    headerWebsite,
    ...
    '',
    'Q&A Pack',
    `Total questions: ${Number.isFinite(questionCount) ? questionCount : 0}`,
  ]
  ...
  appendPredictionTextLine(lines, 'Role summary', interviewBrief.role_summary)
...
function buildPredictionExportFilename(detail) {
  const parts = [
    sanitizePredictionFilePart(detail?.title || 'Custom JD #1'),
    sanitizePredictionFilePart(detail?.company || ''),
    sanitizePredictionFilePart(detail?.prediction_id || detail?.predicted_at || ''),
  ].filter(Boolean)
...
function normalizePredictionDetail(detail) {
  const row = detail || {}
  const legacy = row.legacy_result_json || {}
  const legacyQa = Array.isArray(legacy.qa) ? legacy.qa : []
  const qaPack = row.qa_pack && Object.keys(row.qa_pack || {}).length
    ? row.qa_pack
    : (legacy.qa_pack && Object.keys(legacy.qa_pack || {}).length ? legacy.qa_pack : { total_questions: legacyQa.length, questions: legacyQa })
  const interviewBrief = row.interview_brief && Object.keys(row.interview_brief || {}).length
    ? row.interview_brief
    : (legacy.interview_brief || {})
  const gapDrillPlan = row.gap_drill_plan && Object.keys(row.gap_drill_plan || {}).length
    ? row.gap_drill_plan
    : (legacy.gap_drill_plan || {})
  const sourceSummary = row.source_summary && Object.keys(row.source_summary || {}).length
    ? row.source_summary
...
function predictionConfidenceLabel(value) {
...
function predictionModeLabel(mode) {
  ...
  if (key === 'custom_jd') return 'Custom JD'
  if (key === 'db_job') return 'DB Job'
  return key || '-'
}
...
function predictionBadgeClass(kind, value) {
  const key = String(value || '').trim().toLowerCase()
  if (kind === 'status') {
    if (key === 'completed') return 'inline-badge'
    if (key === 'running') return 'inline-badge inline-badge-warn'
...
function PredictionTextSection({ title, value }) {
  const text = String(value || '').trim()
  if (!text) return null
  return (
    <div className="prediction-section">
...
function AutomationTab({ t, lang, onDataChanged }) {
  ...
  function applyPipelineDefaults(nextType, currentForm) {
    const base = { ...currentForm, pipeline_type: nextType }
    if (nextType === 'artifact_repair_recent') {
      return {
        ...base,
        name: currentForm.name === 'New Schedule' ? 'Nightly Generated Artifact Repair' : currentForm.name,
        frequency: currentForm.frequency === 'every_hours' ? 'daily' : currentForm.frequency,
        run_time: currentForm.run_time || '01:00',
        window_days: Number(currentForm.window_days) || 3,
        max_jobs: Number(currentForm.max_jobs) || 300,
        auto_eval_fit: false,
        auto_generate_cv: false,
        run_now_on_create: false,
      }
    }
    if (nextType === 'garbage_cleanup') {
      return {
        ...base,
        name: currentForm.name === 'New Schedule' ? 'Nightly Garbage Cleanup' : currentForm.name,
        frequency: currentForm.frequency === 'every_hours' ? 'daily' : currentForm.frequency,
        run_time: currentForm.run_time || '01:30',
        window_days: Number(currentForm.window_days) || 10,
        max_jobs: Number(currentForm.max_jobs) || 5000,
        auto_eval_fit: false,
        auto_generate_cv: false,
        run_now_on_create: false,
      }
    }
    if (nextType === 'learning_etl') {
      return {
        ...base,
        auto_eval_fit: false,
        auto_generate_cv: false,
  ...
  const [showManualCrawlConfig, setShowManualCrawlConfig] = useState(() => Boolean(automationView?.showManualCrawlConfig))
  const [manualCrawlTask, setManualCrawlTask] = useState(() => String(automationView?.manualCrawlTask || 'all'))
  const [manualCompanyName, setManualCompanyName] = useState(() => String(automationView?.manualCompanyName || ''))
  ...
  async function startManualFilteredCrawl() {
    const args = ['--window-days', '30']
    if (manualCrawlTask === 'company_jobs') {
      const companyName = String(manualCompanyName || '').trim()
      if (!companyName) {
        window.alert(t.crawlCompanyRequired || 'Enter a company name before running company jobs crawl.')
        return
      }
      args.push('--crawl-task', 'company_jobs', '--company-name', companyName)
    }
    await startManualAction('crawl_filtered', args)
    setShowManualCrawlConfig(false)
  }
  ...
  useEffect(() => {
    writeStoredState(AUTOMATION_STATE_KEY, {
      ...
      schedulePageSize,
      runPageSize,
      showManualCrawlConfig,
      manualCrawlTask,
      manualCompanyName,
    })
  }, [schedulePage, runPage, schedulePageSize, runPageSize, showManualCrawlConfig, manualCrawlTask, manualCompanyName])
  ...
  function scheduleToForm(schedule) {
    ...
    if (baseForm.pipeline_type === 'artifact_repair_recent') {
      baseForm.window_days = Number(crawlConfig.recent_days) || 3
      baseForm.max_jobs = Number(crawlConfig.repair_limit) || 300
      baseForm.auto_eval_fit = false
      baseForm.auto_generate_cv = false
      if (!String(baseForm.run_time || '').trim()) baseForm.run_time = '01:00'
    } else if (baseForm.pipeline_type === 'garbage_cleanup') {
      baseForm.window_days = Number(crawlConfig.retention_days) || 10
      baseForm.max_jobs = Number(crawlConfig.max_delete_entries) || 5000
      baseForm.auto_eval_fit = false
      baseForm.auto_generate_cv = false
      if (!String(baseForm.run_time || '').trim()) baseForm.run_time = '01:30'
    }
  ...
  function buildSchedulePayloadFromForm(overrides = {}, schedule = null) {
    ...
    } else if (payload.pipeline_type === 'artifact_repair_recent') {
      payload.crawl_config = {
        ...payload.crawl_config,
        recent_days: Number(overrides.window_days ?? sourceForm.window_days) || 3,
        repair_limit: Number(overrides.max_jobs ?? sourceForm.max_jobs) || 300,
        only_missing: Boolean(crawlConfig.only_missing ?? true),
        constraint_mode: String(crawlConfig.constraint_mode || 'medium'),
        shutdown_when_completed: Boolean(overrides.shutdown_when_completed ?? sourceForm.shutdown_when_completed),
      }
      payload.auto_eval_fit = false
      payload.auto_generate_cv = false
    } else if (payload.pipeline_type === 'garbage_cleanup') {
      payload.crawl_config = {
        ...payload.crawl_config,
        retention_days: Number(overrides.window_days ?? sourceForm.window_days) || 10,
        max_delete_entries: Number(overrides.max_jobs ?? sourceForm.max_jobs) || 5000,
        shutdown_when_completed: Boolean(overrides.shutdown_when_completed ?? sourceForm.shutdown_when_completed),
      }
      payload.auto_eval_fit = false
      payload.auto_generate_cv = false
  ...
  async function stopRun(run) {
    if (!window.confirm(`${t.stopRun || 'Stop'} run #${run.id}?`)) return
    await api.forceStopRun(run.id)
    await reloadAndNotify()
  }
  ...
  return (
    <div className="grid single">
      ...
      <div className="card">
        ...
        <div className="filters">
          <button disabled={manualActionPending} onClick={() => setShowManualCrawlConfig((prev) => !prev)}>{t.runCrawlFiltered}</button>
        ...
        {showManualCrawlConfig && (
          <div className="filters" style={{ marginTop: 12 }}>
            <label>
              <span>{t.crawlTask || 'Crawl Task'}</span>
              <select value={manualCrawlTask} onChange={(event) => setManualCrawlTask(event.target.value)}>
                <option value="all">{t.crawlAll || 'Crawl All'}</option>
                <option value="company_jobs">{t.crawlCompanyJobs || 'Crawl Company Jobs'}</option>
              </select>
            </label>
            {manualCrawlTask === 'company_jobs' && (
              <label>
                <span>{t.crawlCompanyName || 'Company Name'}</span>
                <input
                  value={manualCompanyName}
                  placeholder={t.crawlCompanyPlaceholder || 'e.g. Google or https://www.linkedin.com/company/google/'}
                  onChange={(event) => setManualCompanyName(event.target.value)}
                />
              </label>
            )}
            <button disabled={manualActionPending || (manualCrawlTask === 'company_jobs' && !String(manualCompanyName || '').trim())} onClick={startManualFilteredCrawl}>
              {t.startCrawlFiltered || 'Start Crawl Filtered'}
            </button>
            <button disabled={manualActionPending} onClick={() => setShowManualCrawlConfig(false)}>
              {t.cancelCrawlFiltered || 'Cancel Crawl'}
            </button>
          </div>
        )}
        </div>
      ...
      <div className="card">
        ...
        <div className="schedule-grid">
          ...
          <label>{t.scheduleType}
            <select value={form.pipeline_type} onChange={(e) => setForm(applyPipelineDefaults(e.target.value, form))}>
              ...
              <option value="artifact_repair_recent">{t.artifactRepairPipeline || 'Generated Artifact Repair'}</option>
              <option value="garbage_cleanup">{t.garbageCleanupPipeline || 'Garbage Cleanup'}</option>
            </select>
          </label>
          <label>{t.frequency}
          ...
          <label>{form.pipeline_type === 'artifact_repair_recent' ? (t.predictRecentDays || 'Recent days') : form.pipeline_type === 'garbage_cleanup' ? (t.retainDays || 'Retain days') : t.windowDays}<input type="number" min="1" max="120" value={form.window_days} onChange={(e) => setForm({ ...form, window_days: e.target.value })} /></label>
          <label>{form.pipeline_type === 'garbage_cleanup' ? (t.maxDeletes || 'Max deletes') : (t.maxJobs || 'Max jobs')}<input type="number" min="1" max="20000" value={form.max_jobs} onChange={(e) => setForm({ ...form, max_jobs: e.target.value })} /></label>
          ...
          <label className="toggle-row"><input type="checkbox" checked={form.auto_eval_fit} disabled={form.pipeline_type === 'artifact_repair_recent' || form.pipeline_type === 'learning_etl' || form.pipeline_type === 'garbage_cleanup'} onChange={(e) => setForm({ ...form, auto_eval_fit: e.target.checked })} />{t.autoEvaluateFit}</label>
          <label className="toggle-row"><input type="checkbox" checked={form.auto_generate_cv} disabled={form.pipeline_type === 'artifact_repair_recent' || form.pipeline_type === 'learning_etl' || form.pipeline_type === 'garbage_cleanup'} onChange={(e) => setForm({ ...form, auto_generate_cv: e.target.checked })} />{t.autoGenerateCv}</label>
            ...
            {runPg.items.map((r) => (
              <tr key={r.id}>
                ...
                <td className="row-actions schedule-action-row">
                  ...
                  <button
                    onClick={() => resumeRun(r)}
                    disabled={String(r.status || '').toLowerCase() !== 'paused'}
                  >
                    {t.resumeRun || 'Resume'}
                  </button>
                  <button
                    onClick={() => stopRun(r)}
                    disabled={String(r.status || '').toLowerCase() !== 'running'}
                  >
                    {t.stopRun || 'Stop'}
                  </button>
                  <button
...
function InterviewQaPredictionTab({ t }) {
  ...
  const hasCustomJd = Boolean(jdText)
  const hasExplicitJobIds = Boolean(String(predictionForm.jobIds || '').trim())
  const customOnlyMode = hasCustomJd && !hasExplicitJobIds
  ...
  async function runPredictionManualAction() {
    ...
    const companyName = String(predictionForm.companyName || '').trim()
    if (companyName) {
      const encodedCompany = encodeBase64Text(companyName)
      if (encodedCompany) {
        args.push('--custom-company-base64', encodedCompany)
      }
    }
    const companyWebsite = String(predictionForm.companyWebsite || '').trim()
    if (companyWebsite) {
      const encodedWebsite = encodeBase64Text(companyWebsite)
      if (encodedWebsite) {
        args.push('--custom-company-website-base64', encodedWebsite)
      }
    }
    ...
    if (runAt) {
      learningQuizLogger.info('Interview QA submit scheduled', {
        ...
        has_company_website: Boolean(companyWebsite),
        custom_only_mode: customOnlyMode,
      })
    } else {
  ...
  return (
    <div className="card">
      ...
      <div className="muted">
        {customOnlyMode
          ? (t.predictCustomOnlyHint || 'Custom JD mode is active. Recent days and Max jobs are ignored unless you also provide Job IDs.')
          : (t.predictDbWindowHint || 'Recent days and Max jobs are only used for loading jobs from the database window.')}
      </div>
      <div className="prediction-grid">
        <label className="action-input-label">
          <span>{t.predictCompanyName || 'Company name'}</span>
          <input
            placeholder="e.g. Example Inc."
            value={predictionForm.companyName}
            onChange={(event) => setPredictionForm((prev) => ({ ...prev, companyName: event.target.value }))}
          />
        </label>
        <label className="action-input-label">
          <span>{t.predictCompanyWebsite || 'Company website'}</span>
          <input
            placeholder="https://example.com"
            value={predictionForm.companyWebsite}
            onChange={(event) => setPredictionForm((prev) => ({ ...prev, companyWebsite: event.target.value }))}
          />
        </label>
      </div>
      <label className="action-input-label">
...
function InterviewQaPredictionResultsTab({ t }) {
  const [manualActionPending, setManualActionPending] = useState(false)
  const [predictionActionMessage, setPredictionActionMessage] = useState('')
  const [predictionActionError, setPredictionActionError] = useState('')
  const [predictionForm, setPredictionForm] = useState({
  ...
  const [predictionDetailError, setPredictionDetailError] = useState('')
  const [predictionDetail, setPredictionDetail] = useState(null)
  const [detailTab, setDetailTab] = useState('qa_pack')

  const jdText = String(predictionForm.jdText || '').trim()
  const hasCustomJd = Boolean(jdText)
  const hasExplicitJobIds = Boolean(String(predictionForm.jobIds || '').trim())
  const customOnlyMode = hasCustomJd && !hasExplicitJobIds
  const detail = normalizePredictionDetail(predictionDetail || {})
  const legacy = detail.legacy_result_json || {}
  const qaPack = detail.qa_pack || legacy.qa_pack || { total_questions: 0, questions: legacy.qa || [] }
  const interviewBrief = detail.interview_brief || legacy.interview_brief || {}
  ...
  async function refreshPredictionRows({ quiet = false } = {}) {
    if (!quiet) setPredictionLoading(true)
    try {
      const res = await api.interviewQaPredictions({ limit: 20, offset: 0 })
      setPredictionRows(Array.isArray(res.items) ? res.items : [])
      setPredictionTotal(Number(res.total || 0))
      setPredictionError('')
    } catch (error) {
      setPredictionError(String(error?.message || error))
    } finally {
      if (!quiet) setPredictionLoading(false)
    }
  }
  ...
  useEffect(() => {
    ...
    const load = async () => {
      if (cancelled) return
      await refreshPredictionRows({ quiet: false })
    }
    load()
    const timerId = window.setInterval(() => {
      refreshPredictionRows({ quiet: true }).catch(() => {})
    }, 12000)
    return () => {
      cancelled = true
  ...
  async function openPredictionDetail(predictionId) {
    ...
    setPredictionDetailError('')
    try {
      const res = await api.interviewQaPredictionDetail(id)
      setPredictionDetail(normalizePredictionDetail(res))
      setDetailTab('qa_pack')
    } catch (error) {
      setPredictionDetail(null)
      setPredictionDetailError(String(error?.message || error))
    } finally {
  ...
  function handleDownloadPredictionTxt() {
    ...
    const text = buildPredictionExportText(detail)
  ...
  async function runPredictionManualAction() {
    ...
    const parsedJobIds = String(predictionForm.jobIds || '')
      ...
      .map((value) => Number(value))
      .filter((value) => Number.isFinite(value) && value > 0)
    parsedJobIds.forEach((jobId) => args.push('--job-id', String(jobId)))
    const cvPath = String(predictionForm.cvPath || '').trim()
    if (cvPath) args.push('--cv-path', cvPath)
    const recentDays = Math.max(0, Number(predictionForm.recentDays) || 0)
    args.push('--recent-days', String(recentDays))
    const limit = Math.max(1, Number(predictionForm.limit) || 1)
    args.push('--limit', String(limit))
    ...
    if (companyName) {
      const encodedCompany = encodeBase64Text(companyName)
      if (encodedCompany) args.push('--custom-company-base64', encodedCompany)
    }
    const companyWebsite = String(predictionForm.companyWebsite || '').trim()
    if (companyWebsite) {
      const encodedWebsite = encodeBase64Text(companyWebsite)
      if (encodedWebsite) args.push('--custom-company-website-base64', encodedWebsite)
    }
    const runAt = toIsoFromDateTimeLocal(predictionForm.startAt)
    setPredictionActionMessage('')
    setPredictionActionError('')
    ...
    try {
      ...
      setPredictionActionMessage(runAt ? `Scheduled predict_interview_qa for ${runAt}` : `Triggered predict_interview_qa${response?.run_id ? ` (run ${response.run_id})` : ''}`)
      refreshPredictionRows({ quiet: true }).catch(() => {})
    } catch (error) {
      setPredictionActionError(String(error?.message || error))
    } finally {
  ...
  return (
    <div className="card">
      ...
      <div className="prediction-grid">
        <label className="action-input-label"><span>{t.predictJobIds || 'Job IDs'}</span><input placeholder={t.predictJobIdsPlaceholder || 'e.g. 4381111111,4382222222'} value={predictionForm.jobIds} onChange={(e) => setPredictionForm((prev) => ({ ...prev, jobIds: e.target.value }))} /></label>
        <label className="action-input-label"><span>{t.predictCvPath || 'CV path'}</span><input placeholder="input/full_doc_stlye.txt" value={predictionForm.cvPath} onChange={(e) => setPredictionForm((prev) => ({ ...prev, cvPath: e.target.value }))} /></label>
        <label className="action-input-label"><span>{t.predictRecentDays || 'Recent days'}</span><input type="number" min="0" value={predictionForm.recentDays} onChange={(e) => setPredictionForm((prev) => ({ ...prev, recentDays: e.target.value }))} /></label>
        <label className="action-input-label"><span>{t.predictMaxJobs || 'Max jobs'}</span><input type="number" min="1" value={predictionForm.limit} onChange={(e) => setPredictionForm((prev) => ({ ...prev, limit: e.target.value }))} /></label>
        <label className="action-input-label"><span>{t.predictStartAt || 'Start job time'}</span><input type="datetime-local" value={predictionForm.startAt} onChange={(e) => setPredictionForm((prev) => ({ ...prev, startAt: e.target.value }))} /></label>
      </div>
      <div className="muted">{customOnlyMode ? (t.predictCustomOnlyHint || 'Custom JD mode is active. Recent days and Max jobs are ignored unless you also provide Job IDs.') : (t.predictDbWindowHint || 'Recent days and Max jobs are only used for loading jobs from the database window.')}</div>
      <div className="prediction-grid">
        <label className="action-input-label"><span>{t.predictCompanyName || 'Company name'}</span><input placeholder="e.g. Example Inc." value={predictionForm.companyName} onChange={(e) => setPredictionForm((prev) => ({ ...prev, companyName: e.target.value }))} /></label>
        <label className="action-input-label"><span>{t.predictCompanyWebsite || 'Company website'}</span><input placeholder="https://example.com" value={predictionForm.companyWebsite} onChange={(e) => setPredictionForm((prev) => ({ ...prev, companyWebsite: e.target.value }))} /></label>
      </div>
      <label className="action-input-label">
        <span>{t.predictJdText || 'JD text'}</span>
        <textarea rows="3" value={predictionForm.jdText} onChange={(e) => setPredictionForm((prev) => ({ ...prev, jdText: e.target.value }))} placeholder="Paste JD text to encode it for prediction" />
      </label>
      <label className="action-input-label">
        <span>{t.predictJdFile || 'JD file'}</span>
        <input type="file" accept=".txt,.md,.json,.docx,.doc" onChange={handlePredictionFile} />
        {predictionForm.jdFileName ? <div className="muted">Loaded: {predictionForm.jdFileName}</div> : null}
        {predictionFileError ? <div className="error-text">{predictionFileError}</div> : null}
      </label>
      <div className="filters">
        <label className="toggle-row"><input type="checkbox" checked={predictionForm.force} onChange={(e) => setPredictionForm((prev) => ({ ...prev, force: e.target.checked }))} /><span>{t.predictForce || 'Force run even when feature flag is off'}</span></label>
        <label className="toggle-row"><input type="checkbox" checked={predictionForm.shutdownWhenCompleted} onChange={(e) => setPredictionForm((prev) => ({ ...prev, shutdownWhenCompleted: e.target.checked }))} /><span>{t.shutdownWhenCompleted || 'Shut down when completed'}</span></label>
        <button disabled={manualActionPending} onClick={runPredictionManualAction}>{t.runGenerateInterviewQa || 'Generate Interview Q&A'}</button>
      </div>
      {predictionActionMessage ? <div className="muted">{predictionActionMessage}</div> : null}
      {predictionActionError ? <div className="error-text">{predictionActionError}</div> : null}
      ...
      <div className="card prediction-results-card">
        <div className="card-head">
          <div><h3>Interview Prediction Results</h3><div className="muted">Latest prediction results for Interview Q&A.</div></div>
          <div className="row-actions-right"><span className="muted">{predictionLoading ? 'Loading...' : `${predictionTotal} records`}</span><button onClick={() => refreshPredictionRows()} disabled={predictionLoading}>Refresh</button></div>
        </div>
        {predictionError ? <div className="error-text">{predictionError}</div> : null}
        {!predictionLoading && predictionRows.length === 0 ? (
          <div className="empty-state"><div className="muted">No completed prediction rows yet.</div></div>
        ) : (
              ...
              <tbody>
                {predictionRows.map((row) => (
                  <tr key={row.prediction_id} className="prediction-row" onClick={() => openPredictionDetail(row.prediction_id)}>
                    <td>{formatGmt7(row.predicted_at, 'en')}</td>
      ...
      >
        {predictionDetailLoading ? (
          ...
          <div className="error-text">{predictionDetailError}</div>
        ) : (
          <div className="prediction-detail">
            <div className="prediction-detail-head">
              <div>
                <div className="prediction-detail-title">{detail.title || '-'}</div>
                <div className="muted">{detail.company || '-'}</div>
                {detail.company_website ? <div className="muted"><a href={detail.company_website} target="_blank" rel="noreferrer">{detail.company_website}</a></div> : null}
              </div>
            ...
            <div className="prediction-tabs">
              ...
              <button className={`subtab-btn ${detailTab === 'gap_drill_plan' ? 'active' : ''}`} onClick={() => setDetailTab('gap_drill_plan')}>Gap & Drill Plan</button>
            </div>
            <div className="prediction-tab-panel">
              {detailTab === 'qa_pack' ? (
                questionCards.length === 0 ? <div className="muted">No Q&A Pack data available.</div> : (
                  <div className="prediction-question-list">
                    <div className="muted">Total questions: {Number(qaPack.total_questions || questionCards.length || 0)}</div>
              ...
              ) : detailTab === 'interview_brief' ? (
                Object.keys(interviewBrief || {}).length === 0 ? <div className="muted">Legacy result does not include brief yet.</div> : (
                ...
                <div className="prediction-detail-section">
                  ...
                  {Array.isArray(gapDrillPlan.mock_plan) && gapDrillPlan.mock_plan.length > 0 ? (
                    <div className="prediction-section">
                      ...
                      <ul className="prediction-list">
                        {gapDrillPlan.mock_plan.map((item, idx) => <li key={`mock_${idx}`}><strong>{item.step || idx + 1}. {item.title || '-'}</strong> ({Number(item.duration_minutes || 0)} min) {item.task || ''}</li>)}
                      </ul>
                    </div>
                  ) : null}
                  <PredictionSectionList title="Final checklist before interview" items={gapDrillPlan.final_checklist_before_interview} />
                </div>
              )}
            </div>
            <details className="prediction-sources" open>
  ...
  return (
    <div className="stack">
      ...
      {subTab === 'knowledge' ? (
        ...
        <InterviewQaPredictionResultsTab t={t} />
      ) : (
        <div className="card">
...
function LogViewerPage() {
  ...
  const [autoRefresh, setAutoRefresh] = useState(() => String(params.get('running') || '').trim() === '1')
  ...
  function toggleAutoRefresh() {
    ...
    if (nextValue) {
      nextParams.set('running', '1')
    } else {
      nextParams.delete('running')
    }
    ...
    <div className="app log-viewer-page">
      <header>
        ...
        <div className="header-right">
          ...
          <button type="button" className="link-btn" onClick={toggleAutoRefresh}>
