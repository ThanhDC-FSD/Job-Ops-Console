...
## 2026-04-08 Modular Runtime Update

The runtime command is unchanged:

```text
.venv\Scripts\python.exe scripts\python\predict_night.py <args...>
```

Internally, the implementation is now split into `scripts/python/interview_qa/` to keep the current business logic but make each stage easier to reason about:

- `job_loader.py` loads SQLite jobs and manual JD text inputs
- `ranking.py` keeps the lightweight-first stage by ranking question themes and retrieving compact evidence chunks
- `prompts.py` assembles smaller prompts so the local Qwen path does not receive raw full-text context
- `llm.py` centralizes backend dispatch and now accepts `gateway_ollama_chat` in addition to direct `ollama` and `llama_cpp`
- `schema.py` validates JSON output and removes unsupported `evidence_ids` before saving
- `reporting.py` writes per-job JSON plus `summary.json` and `summary.md`
- `runner.py` orchestrates the end-to-end flow while `predict_night.py` stays as the compatibility entrypoint used by automation

### Current runtime sequence after the refactor

1. `predict_night.py` parses CLI input and forwards to `interview_qa.runner`
2. `job_loader.py` loads DB jobs and decodes manual JD text
3. `ranking.py` selects top question themes and top evidence chunks
4. `prompts.py` builds the generate prompt
5. `llm.py` calls the configured local backend
6. `schema.py` parses and validates the JSON payload
7. optional vet pass repeats steps 4 to 6 on the smaller generated payload
8. `reporting.py` writes `job_<id>.json`, `summary.json`, and `summary.md`

### Why this is better for weak machines

- Prompt size stays bounded because only shortlisted evidence is sent to the model.
- Validation happens before disk writes, so downstream consumers do not need to re-sanitize bad output.
- Backend dispatch is centralized, which reduces config drift between direct local runs and gateway-based runs.
- The monolithic script is thinner, so isolated tests can run without booting the full automation stack.
