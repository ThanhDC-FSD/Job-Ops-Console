...
- When `fallback_reason != none`, rendered files prepend a warning header and embed `fallback_reason` plus `validator_failure_reason` metadata.
...
- Rendered files include YAML front matter with `schema_version`, `prompt_version`, `retrieval_version`, `validator_version`, `threshold_version`, and `generation_timestamp`.
