            ...
            for searchTerm, job_data in data.items():
                ...
                for job in job_data:
                    try:
                        ...
                        # Existing semantic similarity score
                        if hasattr(self.agent, "evaluate_fit"):
                            try:
                                llm_eval = self.agent.evaluate_fit(job, self.args.resume_txt)
                                logging.info(f"LLM fit evaluation for job %s: %s", job_id, llm_eval)
                                # combine numeric scores (average) to enhance decision
                                combined_score = (float(score) + float(llm_eval.get('total_score', 0.0))) / 2.0
                            except Exception as e:
                                logging.warning("LLM fit evaluation failed: %s", e)
                                llm_eval = None
                        ...
                        if combined_score < self.args.min_score:
                            logging.info(f"Low combined fit score {combined_score} for job {job_id}, skipping.")
                            continue
                        ...
                        self.applied['jobs'][job_id] = {
                            ...
                            'combined_fit_score': combined_score,
                            'llm_fit_eval': llm_eval,
